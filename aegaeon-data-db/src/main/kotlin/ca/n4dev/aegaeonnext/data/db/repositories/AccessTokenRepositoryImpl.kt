...
private const val GET_BY_TOKEN =
    "select id, token, user_id, client_id, scopes, valid_until, version from access_token where token = ?"

private const val GET_BY_USER_ID =
    "select id, token, user_id, client_id, scopes, valid_until, version from access_token where user_id = ?"

private const val COUNT_BY_USER_ID =
    "select count(*) from access_token where user_id = ?"
...
class AccessTokenRepositoryImpl : BaseRepository(), AccessTokenRepository {
    ...
    override fun getByUserId(userId: Long, page: Page): QueryResult<AccessToken> {
        val results = jdbcTemplate.query(GET_BY_USER_ID, mapOf(Pair("user_id", userId)), resultSetToAccessToken)
